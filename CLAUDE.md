...
**NOT closed — two findings larger than a column-triage item, flagged for their own scoped audit:**

1. **`verify-company-registry` — live, daily-scheduled (`verify-company-registry-daily`, jobid 5, `0 10 * * *`, active), writes a public verification claim gated on a computed confidence score.** Checks every `profile_published` startup against OpenCorporates, UK Companies House, and an AI web-search-based DIFC lookup; combines fuzzy-name-match confidence and an LLM-assigned `high|medium|low` DIFC confidence (`scoreMap: {high:45, medium:30, low:15}`) into `maxConf`, then writes `startups.registry_verified = (maxConf >= 60)` — rendered on the public profile at `p.$slug.tsx:114`. This is a live, scheduled, automated system asserting "verified" as fact based on a threshold score, the same shape as the already-stubbed `verify-investor`, except this one is actually running on schedule (checked: `pg_net` is not the blocker here, `verify-company-registry` isn't one of the affected crons). The sweep's original one-line note ("likely legitimate") was not verified against the actual write path — it is not a factual registry lookup alone, it is a scored, thresholded, published verification claim. Not stubbed by this pass — flagged for its own audit given it touches every published startup's public page.

2. **`daily-desk-cron` — 1157-line "Daily Desk" task generator, live via a founder-onboarding trigger, not dormant despite not appearing in `cron.job`.** No pg_cron job invokes it by name or URL — but `seedFounderPlaybook` (`lib/desk-fn.ts:335`), called from the real onboarding route `app.profile-builder.tsx`, hits it directly with a service-role key for every founder completing profile-builder. Contains, in one function: a `founder_fit_score` computation (0–100, scores an investor against what a founder is looking for, mirrors the `deal_briefs.match_score` pattern deliberately — see the comment at line 484) written to `thesis_alerts`; its own independent `matchScore`/`dealScore` deal-brief generation writing to `deal_briefs` — **a second, independent writer of the same table `generate-deal-brief` also writes, an unexamined dual-writer entanglement**; and reads of `thesis_alerts.match_score` for a `priority: alert.match_score >= 85 ? "high" : "normal"` task-priority decision. Not touched by this pass — it's wired to a real onboarding action, is large, and contains at least three distinct scoring computations layered together, which needs its own step-0 trace rather than a triage-item disposition.
...
**Also surfaced, not investigated further — logged so it isn't lost:** `run-founder-coaching` (live caller: `lib/coaching-fn.ts`) reads `readiness_score_runs`/`investor_sim_runs` — both confirmed-dead tables — into a coaching prompt. Since both source tables are dead (their writers stubbed), this function is reading stale-forever data rather than fabricating live scores, but it hasn't been read in full and its own prompt hasn't been checked for scoring/recommendation language. Fifth item for the same future audit pass as #1 and #2 above.
