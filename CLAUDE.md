...
- **Open, blocking the authorization-port step:** (1) reconciling `status` vs. `workflow_stage` as two independently-written lifecycle signals — a product decision, not a code fix; (2) the org-code allocation scheme for reference numbering (§20.6) — designed this session (base-36, 6 characters, dedicated singleton counter `pack_v1.next_org_code()`, distinct from `reference_counter`), not yet built, pending review; (3) the `useStageTransition.ts` interaction flagged above.
