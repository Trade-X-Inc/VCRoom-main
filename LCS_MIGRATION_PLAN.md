...
### Phase 3 — CLOSED (`Dropzone.tsx` → `ProfileBuilder.tsx`)

Both files built, byte-identical logic confirmed via full diff filters, tsc-verified (55/55 exact error-set match, both files together), live-verified where reachable.

- **`Dropzone.tsx`** — the cross-group fix attached to Group 7 per the confirmed decision (its only two callers live inside Group 6's already-closed `app.deal-rooms.$id.documents.tsx`, not any Group 7 file). The confirm-first `MismatchDialog` converted to `LcsModal` (title/body/footer anatomy matched exactly — right-aligned cancel-then-primary footer, same order the original already used); the dropzone drop-area and file-row list restyled onto LCS tokens matching `LcsDropzone`'s documented visual spec (96px-class drop area, dashed `--lcs-line` border, `--lcs-surface` background) — confirmed once more NOT swapped for the actual `LcsDropzone` primitive, since that primitive still has no drag-and-drop, validation, or upload logic of its own. All real logic (drag/drop handlers, the upload→classify→insert sequence, the confirm-first mismatch flow, `logActivity`) confirmed byte-identical by diff. `cn` import removed as dead (no remaining callers after the token conversion). Live-verified against the real Atlas Robotics fixture room's real Information Vault documents tab: dropzone chrome renders correctly (accent upload tile, dashed border, correct copy) above 5 real uploaded documents, zero console errors. No real file was dropped/uploaded during verification — the write path itself was confirmed unchanged by diff, and exercising it would spend a real AI classification call with no need to, per the same reasoning already applied to every other real-write control this migration has left unclicked.
- **`ProfileBuilder.tsx`** (646 lines, `components/founder/`) — kept and restyled per your confirmed decision (its writes are load-bearing for 3 other live files, even though the component itself has zero route importers). All 10 section editors (`ExecutiveSummary`/`Team`/`ProblemSolution`/`Market`/`RevenueModel`/`Traction`/`Financials`/`Legal`/`Product`/`CompetitiveLandscape`) share the same `Field`/`TextArea` input atoms, restyled once to `LcsTextField`'s exact visual spec and propagated automatically to every editor. Section-visibility control (decision 6, confirmed in Group 7's Phase-0 batch) applied here too — `VISIBILITY_CLASSES`' purple/green/gray pill-with-emoji treatment removed entirely, replaced with a plain bordered chip and emoji-free labels, matching `achievements.tsx`'s treatment exactly. Genuine 2-state content-status text (`Empty`/`Complete`) mapped onto `--lcs-ink-muted`/`--lcs-satisfied`, `✓` glyph dropped per §13. All write logic (`saveSection`, `cycleVisibility`'s snap-to-`deal_room` guard, `addCustomSection`, the auto-seed effect, `isContentEmpty`) confirmed byte-identical by diff — zero logic touched.
  - **Two recon bugs left exactly as found, per restyle-only discipline, not fixed**: bug #10, the "Extract from document" button's `console.log`-only handler, and bug #11, the auto-seed effect's swallowed insert error. Both still present, both still logged, neither addressed by this pass.
  - **Verification method differs from every other Phase 3 file, disclosed rather than papered over**: `ProfileBuilder.tsx` (the component) has zero route importers anywhere in the app — confirmed dead by the recon, kept anyway per your decision — so it cannot be reached via normal navigation for a live click-through. Verified instead via the same computed-style probe used for Phase 1's `OnboardingTour.tsx`: every `--lcs-*` token the file uses (`--lcs-white`, `--lcs-line`, `--lcs-ink`, `--lcs-satisfied`/`-wash`, `--lcs-attention`/`-wash`, `--lcs-surface`) confirmed resolving to its correct real value in a live page's own cascade, combined with the byte-identical diff and a final zero-hit sweep for every v1 marker (`text-gray-*`, `bg-gray-*`, `rounded-lg`, `rounded-full`, `text-brand`, `Syne`, `rgba(0,0,0,…)`, all red/green/purple utility classes). If this component is ever reactivated on a real route, a full live click-through is still owed before considering its LCS migration fully proven — this verification level is a documented, deliberate exception, not the standard.

Neither recon bug from `app.documents.tsx`/`app.profile.tsx` applies to either Phase 3 file — both remain open, unfixed, tracked for Phase 4 and 5 respectively.

### Phase 4 — CLOSED (`app.documents.tsx`, alone)

The group's largest file (1,798 lines, 21% of the group by volume), built and verified alone, matching the isolation reasoning Group 6 already applied to this exact file's Group-6-scope sibling. Full read of the file (all 4 views — `document-intake`/`source-files`/`digital-document-vault`/`privacy-settings` — plus the `DocumentEditorModal` sub-component) completed before any edit, per the standing discipline.

- **Palette item A (the group's hardest collapse) applied as recommended**: the 5-value `founder_documents.status` (`empty`/`draft`/`ai_extracted`/`complete`/`needs_review`) collapsed onto the closed 4-value vocabulary via a new `getStatusPillProps()` helper replacing the old `getStatusIcon()`/`getStatusBorderColor()` pair — `draft` and `ai_extracted` both resolve to `in-progress`, but the real distinction between them ("you started this" vs. "AI filled this, check it") survives in the pill's label text, not just its tone, per the recon's own recommendation. The redundant colored left-border (a second encoding of the same status) was dropped now that the pill carries it, per the recon's explicit judgment call.
- **Every other genuine multi-state palette mapped 1:1 or via the same evaluative-collapse reasoning already established in Group 6 and this file's own earlier fixes**: Source Files' 3-value extraction state (failed/extracted/not-yet → attention/satisfied/pending, clean 1:1); the AI feedback `signal` (strong/adequate/weak/critical → satisfied/in-progress/attention, weak and critical sharing one tone since the vocabulary has no third negative tier — same reasoning as Group 6's Strengths/Risks/Flags and this file's own document-card feedback-score badge).
- **Decorative/categorical palettes stripped to plain text, not color-coded**: `Required`/`Optional` (was a red badge — a direct "no red anywhere" violation, now a plain bordered chip, since it's an attribute of the template, not a state); Stage-tier badges ("Deal room"/"Detail pack", previously orange/purple) now plain text, same reasoning as Group 6's `DD_CATEGORY_COLORS` removal — color-coding these would falsely imply one tier is "more attention" than another.
- **Category filter rail restyled per Group 7's Phase-0 decision 3** — deliberately distinct from the app's real navigation styling (a plain bordered segmented control, not the sidebar's active-link treatment), since this is a content filter, not navigation.
- **Two upload-entry buttons, the Custom Document modal, the Employee 1-pager modal, and `DocumentEditorModal` all restyled on LCS tokens directly, not forced into `LcsModal`**: `DocumentEditorModal` has a 3-button footer (Close left-aligned, AI Review + Save right-aligned together) that doesn't fit `LcsModal`'s documented 2-button (secondary, then primary, right-aligned only) contract; the two upload modals have one full-width primary action in the body with no footer button row at all. Both shapes are genuinely different from the anatomy `LcsModal` documents, so building custom shells on the same tokens was correct rather than a compromise — reported as a structural-fit note, not silently forced.
- **Two decorative emoji dropped per §13**: `📎` (paperclip on a file name) → real `FileText` icon; `🏛` (deal-room badge) → plain text. `✦` (sparkle, twice) dropped entirely with no replacement — a pure decoration with no functional role, unlike the `✓`/`→`/numbered markers kept elsewhere in the same file, which are genuine functional indicators. `⚠` (warning, in the investor-flag box) → real `AlertCircle` icon.
- **Both known recon bugs (`app.documents.tsx`'s misleading delete-confirmation text, and `handleAIReview`'s status-demotion of a `complete` document) confirmed still present, neither fixed** — restyle-only discipline held throughout, matching Phase 1–3's treatment of every other logged bug in this group.
- **Byte-identical logic confirmed at the highest rigor this group has used**: beyond the full-file diff filter, every real write-bearing function (`handleFileUpload`, `handleCustomDocumentUpload`, `handleEmployeeOnePagerUpload`, `handleRetryExtraction`, `handleDeleteSourceFile`, plus `DocumentEditorModal`'s `handleSave`/`handleAIReview`) was diffed against HEAD **individually, function-body-only** — all seven confirmed byte-identical. Every raw `supabase.from(...).update/upsert/delete/insert(...)` call shape in the file was also extracted and diffed against HEAD as a set — confirmed identical, catching any write-shape drift the function-level diff might have missed from the inline visibility-toggle writes scattered through the JSX (not inside named handlers).
- **tsc**: 55/55, exact error-set match against baseline — including the 3 pre-existing `AIFeedback`-related type errors in the "coaching hint" JSX, confirmed present before this pass and untouched by it (only their line numbers shifted as edits moved code above them). Build clean, gzip 0.73 MB unchanged, action-split guard passed. Dead imports (`Zap`, `Circle`, `cn`, `LcsModal`) removed after the palette/modal conversions made them unused.
- **Live-verified against the real `test-founder@` fixture across all 4 views plus the document editor modal**: Document Intake (breadcrumb, header, "How it works" strip with real accent-numbered cards, stage guidance box, the visually-distinct category filter rail, both upload-entry buttons, all 17 real document cards with correct Required/Optional and stage-tier labels, all "Not started" since this fixture has zero real `founder_documents` rows — confirmed from the database directly, not assumed); the `DocumentEditorModal` opened on a real template (all 5 fields, amber-asterisk required markers, progress bar, 3-button footer) and closed without saving; Digital Document Vault and Privacy Settings both showing correct empty states. Zero console errors on every view.
- **One verification gap disclosed, not silently assumed covered**: this founder fixture has zero real `founder_documents` rows (confirmed via direct query), so the Source Files/Digital Document Vault row-list rendering, the visibility-toggle switches, and populated-state extraction-status pills were never exercised against real populated data — only their empty states were. The byte-identical-diff verification covers the write logic and JSX structure with full confidence; the *visual* rendering of a populated row was not directly observed. Flagged here rather than claimed as fully covered.

This closes Group 7 Phase 4.

### Phase 5, part 1 — CLOSED (`app.profile-builder.tsx`)

Full read of the file (1,490 lines: `select`/`upload`/`interview`/`extracting`/`confirm`/`done` sub-screens plus `FieldRow`/`SectionHeader`/`PathCard`) completed before any edit. `app.profile.tsx` — the group's final and largest file, reviewed alone per your instruction, matching `nda.tsx`'s end-of-group treatment in Group 6 — is the remaining piece of Phase 5.

- **`InterviewScreen` deliberately left untouched, confirmed by grep, not by memory**: this is the exact chat-bubble shape Group 6.5 deferred, and Group 7's Phase-0 decision 4 said the bubble-radius/typography question must be resolved once, not once per file. A full-file marker sweep after every other screen was converted found every remaining v1 hit (11 lines) falls entirely within `InterviewScreen`'s confirmed function boundaries (lines 1022–1118) — zero leakage into any other screen.
- **`shared style consts` (`card`/`inputBase`/`missingBorder`/`labelStyle`) retoned once, propagating to every screen that uses them** — same pattern as every prior file in this group. One correction caught before it shipped: my first pass gave `inputBase` a fixed `height: 32` (matching `LcsTextField`'s single-line spec), but `FieldRow` reuses the same const for both `<input>` and multi-row `<textarea>` elements via a shared `style` variable — a fixed height would have broken every textarea in the confirm screen. Reverted to the original's actual behavior (`padding` only, no fixed height, letting textareas size by `rows`) before it was verified, not after.
- **Phase-0 decision 10 applied**: `PathCard`'s two icon tints (`#a78bfa`/`#34d399`, implying "I have documents" is somehow different in standing from "Start from scratch") both now `--lcs-ink-muted` — two genuinely equal options, not a preferred-vs-alternate pair.
- **Per-document detection badges** (doc-type unknown/known, confidence low/high — 2 genuine 2-state signals) converted to `LcsStatusPill`: unknown→attention, known→satisfied; low-confidence→attention, high-confidence→pending (the closest fit for a neutral "nothing to flag" state, since LCS has no dedicated "good, no action needed" tone beyond satisfied, which was reserved for the type-detection result itself).
- **Card-hover `translateY(-2px)` lift dropped deliberately, not by oversight**: LCS's flat, shadow-free, sharp-corner language has no lift-on-hover treatment anywhere else in this migration (`LcsCard`, every card built in Groups 6–7) — border-color-only hover is the established pattern, so this wasn't preserved. The Skip button's `onMouseEnter`/`onMouseLeave` color-swap was left static rather than dropped or faked: it swapped between v1's `--faint`/`--muted-foreground`, two token tiers LCS collapses into one (`--lcs-ink-muted`), so the distinction the hover relied on no longer exists in the token system — not a loss, a consequence of a simpler 2-tier text-color model.
- **Recon bug #3 (the `profile-builder-source` constant-slug upsert silently overwriting every uploaded file but the last) confirmed still present, not fixed** — restyle-only discipline held.
- **Byte-identical logic verified at Phase 4's rigor**: all 11 write-bearing functions (`ensureStartupId`, `createSession`, `patchSession`, `handleExtractFromDocuments`, `startInterview`, `advanceInterview`, `sendInterviewAnswer`, `finishInterview`, `populateForm`, `enrichV3`, `handleSave`) diffed against HEAD individually — all identical. Every raw `supabase.from(...).update/upsert/delete/insert(...)` call shape extracted and diffed as a set — identical.
- **tsc**: 55/55, exact error-set match against baseline, including this file's own 3 pre-existing errors (2 `RefObject` nullability mismatches, 1 `TypedExtraction` type mismatch), confirmed present before this pass and untouched. Build clean, gzip 0.73 MB unchanged, action-split guard passed.
- **Live-verified against the real `test-founder@` fixture** (confirmed via direct query to have zero `profile_builder_sessions` rows, so the route correctly lands on `select` rather than redirecting away for an already-confirmed session): the `select` screen (eyebrow, heading, both `PathCard`s with equal icon tints, Skip link) and the `upload` screen (step indicator, dropzone, correctly-disabled submit button) both rendered correctly, zero console errors. **A real verification gap, disclosed rather than assumed covered**: `ConfirmScreen`, `ExtractingScreen`, and the `done` screen were not reached live, since every path into them (`handleExtractFromDocuments`, `startInterview`→`finishInterview`) is a real write and/or a real AI-provider call this pass had no specific need to trigger — the same standard already applied to every other real-write control left unclicked throughout this migration. Their byte-identical-diff verification stands; their live rendering does not.
