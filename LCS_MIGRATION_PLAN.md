...
1. **`components/founder/ProfileBuilder.tsx` (646L) has zero importers repo-wide — confirmed by grep, independently re-verified.** Orphaned by an R10-era removal, documented in the codebase's own comment (`app.documents.tsx:788-792`: *"ProfileBuilder (Digital Profile summary) removed from Document Intake — redundant now that the cover/profile UI lives only on Go Live > Full Digital Profile View"*). It is not functionally redundant, though — its `startup_profile_sections` writes are read live by `MutualDisclosure.tsx`, `app.deal-rooms.$id.information.tsx`, and `achievements.tsx`, so deleting the *editor* doesn't touch the *data*. **This is a Phase-0 decision (below), not resolved by this recon** — restyling confirmed-unreachable UI is waste, but deleting it removes the only editor for data three live surfaces depend on. If resolved as "leave unrestyled, out of scope" (recommended), Group 7's real volume drops to ~7,824 lines.
2. **`components/app/Dropzone.tsx` (315L) is not reachable from any Group 7 file — its only two callers are in `app.deal-rooms.$id.documents.tsx`, which Group 6 already migrated.** It is a live v1 island sitting inside an already-closed-and-verified LCS file today — exactly the shell/tab seam Group 6's own Phase-0 decision 3 existed to prevent, missed because it's a shared component rather than a tab. **Confirmed NOT a duplicate of `LcsDropzone`** (see below) — recommend restyling it in place as a small cross-group fix attached to Group 7, not reopening Group 6 or silently folding it into the §19m purple sweep.
...
**Ten Phase-0 decisions need resolving before build — not decided by this recon:**
1. Is `ProfileBuilder.tsx` dead code left out of scope, or restyled defensively? (Recommend: leave unrestyled, out of scope, logged like `AIChat.tsx` — needs a product/human call given three live readers of its data.)
2. `Dropzone.tsx` scope — restyle as part of Group 7 (recommended) vs. reopening Group 6 vs. folding into §19m.
3. Tab rails / view-switchers — three distinct new shapes not covered by Group 6's breadcrumb-inlining precedent: `app.profile.tsx`'s fallback tab rail, `app.documents.tsx`'s category *filter* rail (not navigation), each proposed as its own in-place restyle rather than extending `LcsPageHeader`. Needs confirmation on whether the two should look deliberately similar or deliberately different, since they do different jobs.
4. **`app.profile-builder.tsx`'s `InterviewScreen` chat bubbles are structurally identical to the shape that got `AIChat.tsx` deferred into Group 6.5** — this collides directly with Group 6.5's open sub-question 1 (chat-bubble radius exception). Recommend resolving 6.5's sub-question 1 first, applying the answer to both, rather than letting Group 7 silently pre-decide it.
5. `OnboardingTour.tsx` is cross-cutting into Group 8 (one of its 3 importers is `app.investor.profile.tsx`) — same shape as `PageGuide.tsx`/`HelpGuide.tsx`, already flagged for possible hoisting. Recommend restyling in Group 7 anyway (2 of 3 callers are here; the Group 8 side-effect is a forward improvement, not a regression), but flagging since it touches a file outside this group.
6. Section-visibility 3-value treatment — the highest-stakes palette call in the group (see above). Needs explicit confirmation before build.
7. `app.profile.tsx`'s print-mode (`mode="view"`) path — LCS documents no print treatment. Flag, don't extrapolate, per §0a.
8. Single-series chart color (`#7C3AED` on `app.profile.tsx`'s analytics chart) — same open question the plan already raises for Group 9's `LazyChart.tsx`; worth deciding once for both.
9. Progress bars (3) and toggle switches (2), no LCS equivalent for either — recommend restyle-in-place per Group 6 decision 4's precedent (log for a future consolidation pass, don't invent a primitive mid-restyle).
10. `app.profile.tsx`'s decorative cover hero (`bg-gradient-mesh` + noise texture) — flat `--lcs-surface`, or a deliberate exception for a marketing-facing profile hero?
...
| 7 | Founder Profile & Documents | ~8,470 (confirmed by read; ~7,824 if `ProfileBuilder.tsx` stays out of scope) | Dependency — retires 11 confirmed alias routes (see recon); largest volume | **Recon complete, 10 Phase-0 decisions open — not yet approved to build** |
