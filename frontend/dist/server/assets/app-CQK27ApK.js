...
import { u as useAuth, s as supabase, a as useI18n } from "./router-Rpa7zDhF.js";
import { formatDistanceToNow } from "date-fns";
import { L as LangSwitcher, T as ThemeToggle } from "./LangSwitcher-CSfxme_H.js";
import { u as useProfile } from "./store-_LLbTvev.js";
import "clsx";
...
function UserMenu() {
  ...
  const [open, setOpen] = useState(false);
  if (!user) return null;
  const initials = user.fullName ? user.fullName.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : user.email.slice(0, 2).toUpperCase();
  return /* @__PURE__ */ jsxs("div", { className: "relative", children: [
    /* @__PURE__ */ jsx(
      "button",
      {
      ...
      /* @__PURE__ */ jsxs("div", { className: "absolute right-0 mt-2 w-[260px] rounded-xl border border-border/60 bg-popover shadow-elev z-40 overflow-hidden", children: [
        /* @__PURE__ */ jsxs("div", { className: "p-3.5 border-b border-border/60", children: [
          /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3", children: [
            /* @__PURE__ */ jsx("div", { className: "grid h-10 w-10 place-items-center rounded-full bg-gradient-brand text-brand-foreground text-sm font-semibold", children: initials }),
            /* @__PURE__ */ jsxs("div", { className: "min-w-0", children: [
              /* @__PURE__ */ jsx("div", { className: "text-sm font-medium truncate", children: user.fullName || user.email }),
              /* @__PURE__ */ jsx("div", { className: "text-xs text-muted-foreground truncate", children: user.email })
            ] })
          ] }),
          /* @__PURE__ */ jsx("div", { className: "mt-2 flex items-center gap-1.5 text-[10px]", children: /* @__PURE__ */ jsx("span", { className: "rounded-full bg-accent px-1.5 py-0.5 text-muted-foreground font-medium", children: user.role }) })
        ] }),
...
function AppShell({ children }) {
  ...
  const { data: startupData } = useQuery({
    ...
    enabled: !!user?.id && !isInvestor,
    queryFn: async () => {
      const { data } = await supabase.from("startups").select("id, company_name, stage").eq("founder_id", user.id).limit(1).maybeSingle();
      return data;
    }
  });
  const { data: dealRoomCount } = useQuery({
    ...
    queryFn: async () => {
      const { count } = await supabase.from("deal_rooms").select("*", { count: "exact", head: true }).eq("startup_id", startupData.id);
      return count ?? 0;
    }
  });
  useEffect(() => {
  ...
  const workspaceName = isInvestor ? user.fullName || "" : startupData?.company_name || profile?.name || "";
  return /* @__PURE__ */ jsxs("div", { className: "min-h-screen bg-background flex", children: [
    /* @__PURE__ */ jsxs("aside", { className: cn("border-r border-border/60 bg-sidebar flex flex-col transition-all", collapsed ? "w-[68px]" : "w-[248px]"), children: [
      ...
      /* @__PURE__ */ jsx("div", { className: "px-3 py-3", children: /* @__PURE__ */ jsxs("div", { className: cn("flex items-center gap-2 rounded-lg border border-border/60 bg-background/60 px-2.5 py-2", collapsed && "justify-center"), children: [
        profile?.logoDataUrl && !isInvestor ? /* @__PURE__ */ jsx("img", { src: profile.logoDataUrl, alt: workspaceName, className: "h-6 w-6 rounded-md object-cover" }) : /* @__PURE__ */ jsx("div", { className: "grid h-6 w-6 place-items-center rounded-md bg-gradient-brand text-[10px] font-semibold text-brand-foreground", children: user.fullName ? user.fullName.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "VR" }),
        !collapsed && /* @__PURE__ */ jsx("div", { className: "flex-1 min-w-0", children: workspaceName ? /* @__PURE__ */ jsxs(Fragment, { children: [
          ...
          /* @__PURE__ */ jsx("div", { className: "text-[10px] text-muted-foreground truncate", children: isInvestor ? "Fund · Partner" : startupData?.stage || profile?.stage || "Company" })
      ...
      /* @__PURE__ */ jsxs("nav", { className: "flex-1 px-2 space-y-0.5 overflow-y-auto", children: [
        ...
        nav.map((n) => {
          ...
          const badge = (() => {
            ...
            if (n.to === "/app/deal-rooms") return dealRoomCount && dealRoomCount > 0 ? String(dealRoomCount) : void 0;
            return n.badge;
          })();
      ...
      /* @__PURE__ */ jsx("div", { className: "p-3 border-t border-border/60", children: collapsed ? /* @__PURE__ */ jsxs(Fragment, { children: [
        /* @__PURE__ */ jsx(
          Link,
          {
            to: "/app/settings",
        ...
        /* @__PURE__ */ jsx("button", { onClick: () => setCollapsed(false), className: "mt-1 w-full grid place-items-center py-2 text-muted-foreground hover:text-foreground", children: /* @__PURE__ */ jsx(ChevronsLeft, { className: "h-4 w-4 rotate-180" }) })
      ] }) : null })
    ] }),
