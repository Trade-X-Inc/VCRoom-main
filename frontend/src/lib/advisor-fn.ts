...
export const sendAdvisorMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }: { data: AdvisorInput }): Promise<AdvisorResult> => {
    ...
    const [startupRes, leadCountRes, meetingCountRes, roomCountRes, activityRes] = await Promise.all([
      adminClient
        .from("startups")
        .select("company_name, sector, stage, funding_target, traction, revenue, team_size")
...
Give specific, actionable fundraising advice based on this real context. Be direct and concise. Max 150 words per response. No generic platitudes.`
    ...
    if (!apiKey) {
      ...
      return {
        reply: startup
          ? `Based on your pipeline (${leadCount} leads, ${meetingCount} in meetings, ${dealRoomCount} deal rooms): Focus on converting your meeting-stage leads first. Follow up within 48h of each meeting with a 3-bullet summary and clear next step. Aim for 1 new warm intro per day from existing connections.`
