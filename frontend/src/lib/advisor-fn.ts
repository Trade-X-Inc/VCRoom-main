...
export const getAIAdvice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }: { data: AdvisorInput }): Promise<AdvisorResult> => {
    ...
    const pageGuidance: Record<string, string> = {
      deal_room: `You are the AI advisor inside a Hockeystick deal room. A deal room is a shared, secure space between a startup founder and an investor. It contains: Document Vault (uploaded files with AI summaries), Q&A (async questions between both parties), Workstation (investor's due diligence, thesis alignment scoring, document review), Team Chat (private internal team messaging), Notes, Activity log, and Meetings. Focus on deal-specific actions. If the user asks about uploading documents, tell them to use the Document Vault tab. If they ask about diligence, point them to the Workstation. If they ask about questions from investors, use the Q&A tab.`,
      ...
      pipeline: `The user is asking about their investor pipeline. Hockeystick tracks VC leads with stages, activities, and notes. Help them prioritize outreach, identify investors to follow up, or analyze their funnel conversion. Be specific about which leads need attention based on their context.`,
      meetings: `The user is asking about meetings. Hockeystick lets you schedule investor meetings, track outcomes, and log notes. Help them prepare for upcoming meetings, summarize past ones, or decide next steps after a meeting.`,
      team: `The user is asking about team/invites. In Hockeystick, founders can invite team members and investors to deal rooms. Investors join via secure invite links and sign NDAs on entry. Help them manage their team or understand the invite flow.`,
      general: `You are the AI advisor for Hockeystick — a VC deal room platform. Key features founders use: Document Vault (secure file sharing), Q&A (async investor questions), Deal Rooms (shared spaces with investors), AI Advisor (fundraising guidance), Pipeline (track investors), Meetings. Key features investors use: Workstation (due diligence), Thesis Alignment (AI-powered document analysis against their investment thesis), Investment Scorecard, Deal Flow, AI Analysis. Help with fundraising strategy, investor relations, and deal room usage.`,
    };
    ...
    const systemPrompt = [
      ...
      "- If they ask about a Hockeystick feature, explain exactly where to find it and how to use it",
      "- If they ask a general fundraising question, answer it with their company context in mind",
      "- Never say 'I don't know' — always give your best guidance and suggest the relevant Hockeystick feature",
