...
export const getAIAdvice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }: { data: AdvisorInput }): Promise<AdvisorResult> => {
    ...
    try {
      ...
      if (supabaseUrl && supabaseKey) {
        const usageResp = await fetch(`${supabaseUrl}/rest/v1/rpc/check_and_increment_ai_usage`, {
          ...
          body: JSON.stringify({ p_user_id: auth.uid, p_feature: "advisor" }),
        });
        if (usageResp.ok) {
    ...
    } catch { /* fail open — the RPC itself is still unauthenticated/unconstrained, see CLAUDE.md §52 */ }
    ...
    const pageGuidance: Record<string, string> = {
      deal_room: `You are the AI advisor inside a Lengdon deal room. A deal room is a shared, secure space between a startup founder and an investor. It contains: Document Vault (uploaded files with AI summaries), Q&A (async questions between both parties), Workstation (investor's due diligence, thesis alignment scoring, document review), Notes, Activity log, and Meetings. Focus on deal-specific actions. If the user asks about uploading documents, tell them to use the Document Vault tab. If they ask about diligence, point them to the Workstation. If they ask about questions from investors, use the Q&A tab.`,
      ...
      meetings: `The user is asking about meetings. Lengdon lets you schedule investor meetings, track outcomes, and log notes. Help them prepare for upcoming meetings, summarize past ones, or decide next steps after a meeting.`,
      team: `The user is asking about team/invites. In Lengdon, founders can invite team members and investors to deal rooms. Investors join via secure invite links and sign NDAs on entry. Help them manage their team or understand the invite flow.`,
      general: `You are the AI advisor for Lengdon — a VC deal flow platform. Key features founders use: Document Vault (secure file sharing with investors), Q&A (async investor questions), Deal Rooms (shared secure spaces with investors), AI Advisor (fundraising guidance), Meetings. Key features investors use: Workstation (due diligence), Thesis Alignment (AI-powered document analysis against their investment thesis), Deal Flow, Watchlist. Help with fundraising strategy, investor relations, and deal room usage. CRITICAL: never describe or direct users to a feature that does not exist in the app — if you are unsure whether something exists, say you don't have that feature rather than inventing one.`,
    };
    ...
    const systemPrompt = [
      ...
      "- If they ask about a Lengdon feature, explain exactly where to find it and how to use it",
      "- If they ask a general fundraising question, answer it with their company context in mind",
      "- Never say 'I don't know' — always give your best guidance and suggest the relevant Lengdon feature",
