...
|---|---|---|---|
| 01 | Page shell | `PageShell.tsx` | `LcsPageShell`. Fixed 200px sidebar, collapses to 52px icon rail via bottom toggle only (persisted to `localStorage`, wrapped in try/catch — private-window safe). 48px top bar, search left-aligned not centred, notification dot (attention tone, no baked-in count), user menu = initials in a plain circle. |
