      ...
      {round.enabled && (
        <div style={{ padding: "16px 18px" }}>
          ...
          <button onClick={() => setShowAntiDil((v) => !v)}
            style={{ fontSize: "11px", color: "var(--faint)", background: "none", border: "none", cursor: "pointer", padding: "0", marginTop: "2px", display: "flex", alignItems: "center", gap: "4px" }}>
          ...
          {showAntiDil && (
            <div style={{ marginTop: "10px" }}>
              ...
              <p style={{ fontSize: "11px", color: "var(--faint)", marginTop: "6px", lineHeight: 1.6 }}>
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
        <thead>
          <tr>
            <th style={{ textAlign: "left", padding: "8px 10px", color: "var(--muted-foreground)", fontWeight: 600, borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>Stakeholder</th>
            {snapshots.map((s) => (
              <th key={s.stageName} style={{ textAlign: "right", padding: "8px 10px", color: "var(--muted-foreground)", fontWeight: 600, borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>{s.stageName}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {snapshots[0].founders.map((f, fi) => (
            <tr key={f.name} style={{ background: fi % 2 === 0 ? "var(--accent)" : "transparent" }}>
              <td style={{ padding: "8px 10px", display: "flex", alignItems: "center", gap: "6px" }}>
                ...
                <span style={{ color: "var(--muted-foreground)" }}>{f.name}</span>
              </td>
              {snapshots.map((s) => {
          ...
          <tr>
            <td style={{ padding: "8px 10px", display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <span style={{ color: "var(--muted-foreground)" }}>Option pool</span>
            </td>
            {snapshots.map((s) => (
              <td key={s.stageName} style={{ textAlign: "right", padding: "8px 10px", fontVariantNumeric: "tabular-nums", color: "var(--muted-foreground)" }}>
                {fmtPct(s.poolPct)}
              </td>
            ))}
          </tr>
          ...
          {snapshots.filter((s) => s.investorPct > 0.01).map((s, i) => (
            <tr key={s.investorName}>
              <td style={{ padding: "8px 10px", display: "flex", alignItems: "center", gap: "6px" }}>
                ...
                <span style={{ color: "var(--muted-foreground)" }}>{s.investorName} investors</span>
              </td>
              {snapshots.map((snap) => {
                ...
                return (
                  <td key={snap.stageName} style={{ textAlign: "right", padding: "8px 10px", fontVariantNumeric: "tabular-nums", color: snapIdx < baseIdx ? "var(--faint)" : "var(--muted-foreground)" }}>
...
function DilutionPage() {
  ...
  const barSegments = [
    ...latest.founders.map((f, i) => ({ color: FOUNDER_COLORS[i] ?? "var(--brand)", pct: f.pct, label: f.name })),
    ...
    <div style={{ background: "var(--background)", minHeight: "100vh", color: "var(--foreground)" }}>
      ...
      <section style={{ ...pw, padding: "56px 24px 48px" }}>
        <nav style={{ fontSize: "12px", color: "var(--faint)", marginBottom: "20px" }}>
          ...
          <span style={{ color: "var(--muted-foreground)" }}>Dilution Calculator</span>
        </nav>
        ...
        <p style={{ fontSize: "12px", color: "var(--faint)" }}>Percentage-based — no share counts required</p>
      </section>
      ...
      <section style={{ ...pw, paddingBottom: "80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: "24px" }} className="dil-grid">
            ...
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
              ...
              {founders.length < 5 && (
                <button onClick={addFounder}
                  style={{ fontSize: "12px", color: "var(--brand)", background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.2)", borderRadius: "6px", padding: "6px 12px", cursor: "pointer", marginTop: "4px" }}>
                  + Add co-founder
                </button>
              )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            {/* Summary card */}
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", color: "var(--faint)", textTransform: "uppercase", marginBottom: "14px" }}>Founder ownership summary</p>
              ...
              {/* Stacked bar */}
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", color: "var(--faint)", textTransform: "uppercase", marginBottom: "8px" }}>Current cap table</p>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", marginTop: "10px" }}>
                {barSegments.filter((s) => s.pct > 0.05).map((s) => (
                  <div key={s.label} style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                    ...
                    <span style={{ fontSize: "11px", color: "var(--muted-foreground)" }}>{s.label} ({fmtPct(s.pct)})</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Per-founder detail */}
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", color: "var(--faint)", textTransform: "uppercase", marginBottom: "14px" }}>Founder ownership over rounds</p>
              {founders.map((f, fi) => {
            ...
            {/* Waterfall table */}
            <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
              <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", color: "var(--faint)", textTransform: "uppercase", marginBottom: "14px" }}>Dilution waterfall</p>
            ...
            {latest.postMoney > 0 && (
              <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "12px", padding: "20px" }}>
                <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.1em", color: "var(--faint)", textTransform: "uppercase", marginBottom: "14px" }}>Breakeven analysis</p>
                ...
                <p style={{ fontSize: "12px", color: "var(--faint)", lineHeight: 1.6 }}>
                  Founders own <span style={{ color: "var(--foreground)" }}>{fmtPct(founderFinal)}</span> of the company, valued at <span style={{ color: "var(--foreground)" }}>{fmt$(latest.postMoney * founderFinal / 100)}</span> at this valuation.
                </p>
                ...
                {(() => {
                  ...
                  const bSegs = [
                    ...breakevenSnap.founders.map((f, i) => ({ color: FOUNDER_COLORS[i] ?? "var(--brand)", pct: f.pct, label: f.name })),
                  ...
                  return (
                    <div style={{ marginTop: 16 }}>
                      <p style={{ fontSize: 12, color: 'var(--muted-foreground)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
      ...
      <section className="tool-no-print" style={{ borderTop: "1px solid var(--border)", padding: "72px 24px" }}>
        <div style={{ maxWidth: "720px", margin: "0 auto" }}>
          ...
          <Accordion title="Anti-dilution protection">
            <p style={{ fontSize: "14px", color: "var(--muted-foreground)", lineHeight: 1.7, marginBottom: "10px" }}>
            ...
            <p style={{ fontSize: "13px", color: "var(--muted-foreground)", lineHeight: 1.7, marginTop: "10px" }}>
      ...
      <section className="tool-no-print" style={{ borderTop: "1px solid var(--border)", padding: "72px 24px" }}>
        <div style={{ maxWidth: "720px", margin: "0 auto" }}>
          ...
          {[
            ...
            <Accordion key={q} title={q}><p style={{ fontSize: "14px", color: "var(--muted-foreground)", lineHeight: 1.7 }}>{a}</p></Accordion>
          ))}
        </div>
      </section>
