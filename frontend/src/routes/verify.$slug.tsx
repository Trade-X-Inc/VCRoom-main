...
function StatusPill({ state }: { state: "pass" | "fail" | "pending" }) {
  ...
  return (
    <span className="text-xs px-2 py-0.5 rounded-full shrink-0" style={{ background: "var(--accent)", color: "var(--faint)" }}>
      Not checked
    </span>
  );
}
...
function FactLine({ check }: { check: PublicCheckLine }) {
  return (
    <div className="flex items-start justify-between gap-4 py-3.5 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
      <div className="flex-1 min-w-0">
        <div className="text-sm text-muted-foreground">{check.label}</div>
        {check.detail && <div className="text-xs text-faint mt-0.5 leading-relaxed">{check.detail}</div>}
      </div>
  ...
  return (
    <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <ShieldCheck size={16} style={{ color: met ? "#10B981" : "var(--faint)" }} />
        <span style={{ fontFamily: "Syne, sans-serif", fontWeight: 600, fontSize: 13, color: met ? "#fff" : "var(--muted-foreground)" }}>
          {title}
        </span>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{ fontSize: 11, color: met ? "#10B981" : "var(--faint)" }}>
...
const TIER_BADGE: Record<number, { label: string; bg: string; color: string; border: string }> = {
  0: { label: "Not verified", bg: "var(--accent)", color: "var(--muted-foreground)", border: "var(--border)" },
  1: { label: "✓ Identity Confirmed", bg: "var(--accent)", color: "var(--muted-foreground)", border: "var(--border)" },
    ...
    return (
      <div style={{ background: "var(--background)", minHeight: "100vh" }}>
        ...
        <main style={{ maxWidth: 600, margin: "0 auto", padding: "80px 24px", textAlign: "center" }}>
          ...
          <Link to="/" style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "var(--brand)", fontSize: 14, textDecoration: "none" }}>
    ...
    <div style={{ background: "var(--background)", minHeight: "100vh" }}>
      ...
      <main style={{ maxWidth: 680, margin: "0 auto", padding: "64px 24px 96px" }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontFamily: "Syne, sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", color: "var(--faint)", textTransform: "uppercase", marginBottom: 12 }}>
            Verification report
          </div>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div>
              <h1 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 28, color: "var(--foreground)", margin: 0 }}>
        ...
        <p style={{ fontSize: 11, color: "var(--faint)", marginBottom: 32 }}>
        ...
        <div style={{ background: "var(--card)", border: `1px solid ${data.tier1.passed ? "rgba(16,185,129,0.25)" : "var(--accent)"}`, borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
          ...
          {data.tier1.registry_source && (
            <div style={{ padding: "10px 20px", borderTop: "1px solid var(--border)", fontSize: 12, color: "var(--muted-foreground)" }}>
        ...
        <div style={{ background: "var(--card)", border: `1px solid ${data.tier2.met ? "rgba(16,185,129,0.25)" : "var(--accent)"}`, borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
          ...
          {data.tier2.verified_claims.length > 0 ? (
            <div style={{ padding: "0 20px" }}>
              {data.tier2.verified_claims.map((c, i) => (
                <div key={i} className="flex items-start justify-between gap-4 py-3.5 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                  <div className="flex-1 min-w-0">
              ...
              {data.tier2.total_claims > data.tier2.verified_claims.length && (
                <div style={{ padding: "10px 0 14px", fontSize: 11, color: "var(--faint)" }}>
            ...
            <div style={{ padding: "14px 20px", fontSize: 12, color: "var(--faint)" }}>
        ...
        <div style={{ background: "var(--card)", border: `1px solid ${data.tier3.met ? "rgba(59,130,246,0.25)" : "var(--accent)"}`, borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: "0 20px" }}>
            {data.tier3.docs.map((d) => (
              <div key={d.label} className="flex items-start justify-between gap-4 py-3.5 border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-muted-foreground">{d.label}</div>
                  {d.uploaded_at && <div className="text-xs text-faint mt-0.5">Submitted {fmt(d.uploaded_at)}</div>}
                </div>
                <StatusPill state={d.verified ? "pass" : d.uploaded_at ? "fail" : "pending"} />
              </div>
            ))}
          </div>
          <div style={{ padding: "10px 20px", borderTop: "1px solid var(--border)", fontSize: 12, color: data.tier3.human_reviewed ? "#3B82F6" : "var(--faint)", display: "flex", alignItems: "center", gap: 6 }}>
            <User size={12} />
        ...
        <div style={{ background: "var(--card)", border: `1px solid ${data.tier4.signed_off ? "rgba(124,58,237,0.3)" : "var(--accent)"}`, borderRadius: 12, marginBottom: 40, overflow: "hidden" }}>
          <TierHeader title="Tier 4 — Hockystick Verified" met={data.tier4.signed_off} checkedAt={data.tier4.reviewed_at} />
          <div style={{ padding: "14px 20px", fontSize: 12, color: data.tier4.signed_off ? "var(--muted-foreground)" : "var(--faint)", lineHeight: 1.6 }}>
        ...
        <div style={{ textAlign: "center" }}>
          <p style={{ fontSize: 11, color: "var(--faint)", lineHeight: 1.7, marginBottom: 16 }}>
          ...
          <Link to="/" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--faint)", textDecoration: "none" }}>
