...
// ─────────────────────────────────────────────────────────────────────────────
// RESTORED 8 Sep 2026 — this is the real, complete multi-page /docs system
// (layout + registry-driven content, built 8 Jul 2026, last touched 25 Aug
// 2026) that was deleted from routing on 31 Aug 2026 during the founder's
// Figma-fidelity public-site rebuild (commit 1eb3a22) — LENGDONPUBLIC-NEW
// only designed one flat docs page, so the rebuild replaced this shell's
// routing with that flat page and flagged the mismatch in
// DELETED-PUBLIC-ROUTES.md rather than silently dropping it. The content
// modules (docs/content/*.tsx), nav.ts, registry.tsx, and DocArticle.tsx
// were never deleted — only these three route files were. Restored here
// byte-for-byte from `git show 1eb3a22^:frontend/src/routes/docs.tsx`,
// staged at /docs-v2 (not /docs) pending a full accuracy pass on all 40
// pages and a decision on how this coexists with the current flat
// /docs — see CLAUDE.md's docs-restoration entry for the full record.
//
// Original header comment, preserved below for provenance:
// ─────────────────────────────────────────────────────────────────────────────
...
function slugFromPath(pathname: string): string {
  return pathname.replace(/^\/docs-v2\/?/, "").replace(/\/$/, "");
}
              ...
              {s.items.map((item) => {
                ...
                return (
                  <li key={item.slug || "index"}>
                    <Link
                      to={(item.slug ? `/docs-v2/${item.slug}` : "/docs-v2") as any}
    ...
    <div style={{ minHeight: "100vh", background: SURFACE, color: INK }}>
      ...
      <div style={{ margin: "0 auto", maxWidth: "80rem", display: "flex", padding: "0 24px" }}>
        ...
        <main id="main-content" className="lg:px-10" style={{ minWidth: 0, flex: 1, padding: "32px 0 32px 40px" }}>
          ...
          >
            <Link to={"/docs-v2" as any} style={{ color: INK_3, textDecoration: "none" }}>
              Docs
            </Link>
            {section && slug && (
              <>
          ...
          {(prev || next) && (
            ...
            >
              {prev ? (
                <Link
                  to={(prev.slug ? `/docs-v2/${prev.slug}` : "/docs-v2") as any}
              ...
              {next && (
                <Link
                  to={(next.slug ? `/docs-v2/${next.slug}` : "/docs-v2") as any}
