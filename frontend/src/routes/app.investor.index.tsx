...
const STATUS_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  Passed:    { bg: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)" },
  ...
  if (alerts.length === 0) {
    return (
      <ChatResultCard icon={<AlertCircle size={14} />} title="Thesis Alerts">
        <p style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", margin: 0 }}>No thesis alerts yet.</p>
      </ChatResultCard>
    );
  }
  return (
          ...
          return (
            <div key={a.id}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: "#fff" }}>{a.startupName}</span>
              ...
              {a.matchReasons.length > 0 && (
                <ul style={{ margin: "0 0 4px 14px", padding: 0, listStyle: "disc" }}>
                  {a.matchReasons.slice(0, 3).map((r, i) => (
                    <li key={i} style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", lineHeight: 1.5 }}>{r}</li>
                  ))}
                </ul>
              )}
              {a.profileSlug && (
      ...
      <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
        {rooms.map((r) => (
          <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 13, fontWeight: 500, color: "#fff" }}>{r.companyName}</span>
            <a
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#fff", marginBottom: 6, lineHeight: 1.4 }}>
          {brief.headline}
        </div>
      ...
      {Object.keys(brief.keyMetrics).length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 12px", marginBottom: 12, background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: "10px 12px" }}>
          {Object.entries(brief.keyMetrics).slice(0, 6).map(([k, v]) => (
            <div key={k}>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>{k.replace(/_/g, " ")}</div>
      ...
      {brief.strengths.length > 0 && (
        <div style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.18)", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 14 }}>
            {brief.strengths.map((s, i) => <li key={i} style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", lineHeight: 1.5 }}>{s}</li>)}
          </ul>
        </div>
      )}
      ...
      {brief.redFlags.length > 0 && (
        <div style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.18)", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 14 }}>
            {brief.redFlags.map((r, i) => <li key={i} style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", lineHeight: 1.5 }}>{r}</li>)}
          </ul>
        </div>
      )}
      ...
      {brief.suggestedQuestions.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)", marginBottom: 5, textTransform: "uppercase" as const, letterSpacing: "0.06em" }}>Questions to ask</div>
          <ol style={{ margin: 0, paddingLeft: 16 }}>
            {brief.suggestedQuestions.map((q, i) => <li key={i} style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", lineHeight: 1.6, marginBottom: 2 }}>{q}</li>)}
          </ol>
        </div>
      )}
      ...
      {brief.overallVerdict && (
        <div style={{ marginTop: 10, fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.6, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
          {brief.overallVerdict}
        </div>
      )}
    </ChatResultCard>
...
function ConfirmClearDialog({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  return (
    ...
    >
      <div
        style={{ background: "#111114", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: "28px 28px 24px", maxWidth: 380, width: "100%" }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ fontSize: 15, fontWeight: 600, color: "#fff", marginBottom: 8 }}>Clear conversation?</div>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.7)", border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>Cancel</button>
...
function InvestorChat() {
  ...
  const stateActionButton = (firstLoadState === "no-profile" || firstLoadState === "no-thesis") ? (
    <button
      ...
      style={{ background: "#7C3AED", color: "#fff", border: "none", borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 600, cursor: "pointer", marginTop: 10, display: "inline-flex", alignItems: "center", gap: 6 }}
    >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" as const, height: "calc(100vh - 4rem)", background: "#0A0A0B" }}>
      {/* Header */}
      <div style={{ borderBottom: "1px solid rgba(255,255,255,0.08)", background: "rgba(10,10,11,0.85)", backdropFilter: "blur(16px)", padding: "14px 24px", flexShrink: 0 }}>
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg, #7C3AED, #A855F7)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 0 12px rgba(124,58,237,0.4)" }}>
              <Sparkles size={16} color="#fff" />
            </div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#fff", fontFamily: "Syne, sans-serif" }}>AI Advisor</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 1 }}>Investment analysis — thesis scoring, diligence, term sheets</div>
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ position: "relative" as const }} ref={menuRef}>
              <button
                ...
                style={{ width: 32, height: 32, borderRadius: 8, background: menuOpen ? "rgba(255,255,255,0.08)" : "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.5)" }}
              >
                <MoreHorizontal size={16} />
              </button>
              {menuOpen && (
                <div style={{ position: "absolute" as const, top: 38, right: 0, background: "#18181C", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 10, padding: "6px 4px", minWidth: 180, zIndex: 20, boxShadow: "0 8px 24px rgba(0,0,0,0.5)" }}>
                  <button
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 24px 8px" }}>
          {msgs.map((m, idx) => {
            ...
            return (
              <div key={m.id} style={{ display: "flex", flexDirection: isUser ? "row-reverse" : "row", gap: 10, marginBottom: 18, alignItems: "flex-start" }}>
                {/* Avatar */}
                <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: isUser ? "linear-gradient(135deg, #7C3AED, #A855F7)" : "#1A1A1E", border: isUser ? "none" : "1px solid rgba(255,255,255,0.10)", fontSize: 11, fontWeight: 700, color: isUser ? "#fff" : "#A855F7" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 8, maxWidth: "78%", alignItems: isUser ? "flex-end" : "flex-start" }}>
                  ...
                  {/* Gate CTA */}
                  {m.isGateBlock && (
                    <button onClick={() => navigate({ to: "/app/investor/profile" as any })} style={{ background: "#7C3AED", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", alignSelf: "flex-start" }}>
                      Complete my profile →
                    </button>
                  )}
          ...
          {thinking && (
            <div style={{ display: "flex", gap: 10, marginBottom: 18, alignItems: "flex-start" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#1A1A1E", border: "1px solid rgba(255,255,255,0.10)", color: "#A855F7" }}>
                <Sparkles size={13} />
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
              {SUGGESTION_CHIPS.map((chip) => (
                <button
                  key={chip}
                  onClick={() => send(chip)}
                  style={{ background: "#111114", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 12px", textAlign: "left" as const, cursor: "pointer", transition: "border-color 0.15s, background 0.15s" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(124,58,237,0.35)"; (e.currentTarget as HTMLButtonElement).style.background = "#18181C"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "rgba(255,255,255,0.08)"; (e.currentTarget as HTMLButtonElement).style.background = "#111114"; }}
                >
                  <Sparkles size={12} color="#A855F7" style={{ marginBottom: 4 }} />
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.75)", lineHeight: 1.4 }}>{chip}</div>
                </button>
              ))}
            </div>
          )}
      ...
      {/* Composer */}
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", background: "rgba(10,10,11,0.85)", backdropFilter: "blur(16px)", flexShrink: 0, padding: "14px 24px 18px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          {/* Attached file pill */}
          {attachedFile && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(124,58,237,0.1)", border: "1px solid rgba(124,58,237,0.25)", borderRadius: 6, padding: "4px 10px", fontSize: 12, color: "rgba(255,255,255,0.7)" }}>
                <Paperclip size={11} color="#A855F7" />
                {attachedFile.name}
                <button onClick={() => setAttachedFile(null)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex", color: "rgba(255,255,255,0.4)" }}>
                  <XIcon size={12} />
                </button>
              </div>
            </div>
          ...
          <form
            ...
            style={{ display: "flex", alignItems: "flex-end", gap: 8, background: "#111114", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 14, padding: "8px 10px 8px 10px" }}
            onFocus={(e) => { (e.currentTarget as HTMLFormElement).style.borderColor = "rgba(124,58,237,0.5)"; }}
            onBlur={(e) => { (e.currentTarget as HTMLFormElement).style.borderColor = "rgba(255,255,255,0.10)"; }}
          >
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              style={{ width: 32, height: 32, borderRadius: 7, background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.35)", flexShrink: 0 }}
            >
              <Paperclip size={15} />
            </button>
            <textarea
            ...
            <button
              ...
              style={{ width: 34, height: 34, borderRadius: 9, background: (input.trim() || attachedFile) && !thinking ? "linear-gradient(135deg, #7C3AED, #A855F7)" : "rgba(255,255,255,0.08)", border: "none", cursor: (input.trim() || attachedFile) && !thinking ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.15s" }}
            >
              <Send size={14} color={(input.trim() || attachedFile) && !thinking ? "#fff" : "rgba(255,255,255,0.3)"} />
            </button>
          </form>
          <div style={{ marginTop: 6, fontSize: 10, color: "rgba(255,255,255,0.25)", textAlign: "center" as const }}>
