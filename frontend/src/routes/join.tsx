...
function JoinTeamPage() {
  ...
  async function loadInvite() {
    ...
    const { data, error } = await supabase
      .from("team_invites")
      .select("id, token, startup_id, email, role, invited_by, expires_at, accepted_at, startups(company_name, founder_name)")
      .eq("token", token!)
      .maybeSingle();
  ...
  async function handleAccept() {
    ...
    try {
      ...
      // 3. Upsert team_member_profiles (creates empty record)
      await supabase.from("team_member_profiles").upsert({ user_id: user.id }, { onConflict: "user_id", ignoreDuplicates: true });
  ...
  async function handleAuth() {
    ...
    try {
      if (authMode === "signup") {
        ...
        if (error) throw error;
        // user will be populated via auth context → re-render will reach accept step
      }
    } catch (e: any) {
  ...
  const companyName = invite?.startups?.company_name ?? "the company";
    ...
    return (
      <PublicShell>
        <ErrorCard
          ...
          message="This invitation link is invalid. Ask the company to send a new invitation."
      ...
      <div style={{ maxWidth: 420, margin: "0 auto" }}>
        ...
        <div style={{ background: "#111114", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 28, marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.6, marginBottom: 16 }}>
            {invite?.startups?.founder_name ?? "The team"} invited you to join{" "}
        ...
        {user ? (
          ...
          <div style={{ background: "#111114", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 24, textAlign: "center" }}>
            ...
            >
              ...
              Accept and join {companyName} as {roleLabel}
            </button>
          </div>
        ) : (
          // Not logged in — show signup/signin form
