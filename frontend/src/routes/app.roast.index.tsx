...
const STATUS_PILL: Record<string, { label: string; text: string; bg: string }> = {
  ...
  cancelled: { label: "Cancelled", text: "var(--lcs-ink-muted)", bg: "rgba(113,113,122,0.1)" },
};
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: 12, fontWeight: 500, color: "var(--lcs-ink)", background: "var(--lcs-white)",
        border: `1px solid var(--lcs-line)`, borderRadius: "var(--radius-lcs-control)", cursor: "pointer",
        ...style,
      }}
    />
  );
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "none", borderRadius: "var(--radius-lcs-control)", cursor: "pointer",
        ...style,
      }}
    />
  );
...
export function RoastManagement({ view }: { view?: "reports" } = {}) {
  ...
  return (
    <div style={{ maxWidth: 1360, margin: "0 auto", padding: 32 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: "var(--font-lcs-ui)", fontSize: 28, fontWeight: 700, color: "var(--lcs-ink)", display: "flex", alignItems: "center", gap: 8, margin: 0 }}>
            <Flame style={{ width: 24, height: 24, color: "#DC2626" }} />
            {view === "reports" ? "Founder Roast Reports" : "Founder Roast"}
          </h1>
          <p style={{ fontSize: 13, color: "var(--lcs-ink-muted)", marginTop: 4 }}>
      ...
      {!startup?.id && (
        <div style={{ border: `1px solid var(--lcs-line)`, background: "var(--lcs-white)", padding: 32, textAlign: "center", fontSize: 13, color: "var(--lcs-ink-muted)" }}>
          Build your{" "}
      ...
      {view !== "reports" && startup?.id && (
        <div style={{ marginBottom: 24, border: `1px solid var(--lcs-line)`, background: "var(--lcs-white)" }}>
          <button
            type="button"
          ...
          >
            <span style={{ fontFamily: "var(--font-lcs-ui)", fontSize: 14, fontWeight: 700, color: "var(--lcs-ink)" }}>
              What is a Founder Roast?
            </span>
            <ChevronDown style={{ width: 16, height: 16, color: "var(--lcs-ink-muted)", transform: isOverviewOpen ? "rotate(180deg)" : undefined, transition: "transform 0.15s" }} />
          </button>
          {isOverviewOpen && (
            <div style={{ padding: "0 20px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
              <p style={{ fontSize: 13, color: "var(--lcs-ink-muted)", lineHeight: 1.6, margin: 0 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                {[
                  ...
                  <div key={title} style={{ border: `1px solid var(--lcs-line)`, padding: 14 }}>
                    <Icon style={{ width: 14, height: 14, color: "#DC2626", marginBottom: 8 }} />
                    <div style={{ fontSize: 13, fontWeight: 500, color: "var(--lcs-ink)" }}>{title}</div>
                    <div style={{ fontSize: 12, color: "var(--lcs-ink-muted)", marginTop: 4, lineHeight: 1.5 }}>{body}</div>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 8, background: "rgba(5,150,105,0.06)", border: "1px solid rgba(5,150,105,0.2)", padding: 12 }}>
                ...
                <p style={{ fontSize: 12, color: "var(--lcs-ink-muted)", lineHeight: 1.5, margin: 0 }}>
      ...
      {view !== "reports" && showSchedule && (
        <div style={{ marginBottom: 24, border: `2px solid rgba(220,38,38,0.4)`, background: "var(--lcs-white)", padding: 20 }}>
          <div style={{ fontFamily: "var(--font-lcs-ui)", fontSize: 14, fontWeight: 700, color: "var(--lcs-ink)", marginBottom: 16 }}>
            Schedule your Roast
          </div>
          ...
          {scheduleStep === "details" ? (
            <>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: selected ? "2px solid #DC2626" : `1px solid var(--lcs-line)`,
                        background: selected ? "rgba(220,38,38,0.04)" : "var(--lcs-white)",
                      }}
                    >
                      <div style={{ fontFamily: "var(--font-lcs-ui)", fontSize: 13, fontWeight: 700, color: "var(--lcs-ink)" }}>Level {l}</div>
                      <div style={{ fontSize: 12, color: "var(--lcs-ink-muted)", marginTop: 4 }}>
                        Up to {c.maxAudience} challengers · {c.qaMinutes} min Q&amp;A
                      </div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "var(--lcs-ink)", marginTop: 8 }}>${c.priceUsd}</div>
                    </button>
                  );
                })}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
                <div>
                  <label style={{ display: "block", fontSize: 12, color: "var(--lcs-ink-muted)", marginBottom: 4 }}>Date</label>
                  <input
                    type="date"
                    value={dateStr}
                    onChange={(e) => setDateStr(e.target.value)}
                    min={new Date().toISOString().slice(0, 10)}
                    style={{ width: "100%", height: 36, border: `1px solid var(--lcs-line)`, background: "var(--lcs-white)", padding: "0 10px", fontSize: 13 }}
                  />
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 12, color: "var(--lcs-ink-muted)", marginBottom: 4 }}>Time (your local time)</label>
                  <input
                    type="time"
                    value={timeStr}
              ...
              <div style={{ background: "rgba(220,38,38,0.05)", border: "1px solid rgba(220,38,38,0.2)", padding: 14, marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--lcs-ink-muted)", lineHeight: 1.6 }}>
                    <span style={{ fontWeight: 600, color: "var(--lcs-ink)" }}>The Roast rules — read before scheduling.</span>
                    <ul style={{ margin: "6px 0 0", paddingLeft: 16 }}>
                      <li>The session is public and stays public until your funding round closes.</li>
                      <li><span style={{ fontWeight: 600, color: "var(--lcs-ink)" }}>You cannot delete or hide questions — ever.</span> Only platform moderation can remove abuse, and removals are publicly counted.</li>
                ...
                <label style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500, color: "var(--lcs-ink)" }}>I understand and accept these rules.</span>
                </label>
              </div>
      ...
      {(() => {
        ...
        return isLoading ? (
          <div style={{ border: `1px solid var(--lcs-line)`, background: "var(--lcs-white)", padding: 32, textAlign: "center" }}>
            <Loader2 style={{ width: 20, height: 20, color: "var(--lcs-ink-muted)" }} className="animate-spin mx-auto" />
          </div>
        ) : visibleSessions.length === 0 && startup?.id ? (
          <div style={{ border: `1px dashed var(--lcs-line)`, background: "var(--lcs-white)", padding: 40, textAlign: "center" }}>
            <Flame style={{ width: 32, height: 32, color: "rgba(220,38,38,0.35)", margin: "0 auto 12px" }} />
            <div style={{ fontSize: 14, fontWeight: 500, color: "var(--lcs-ink)" }}>{view === "reports" ? "No completed Roasts yet" : "No Roasts yet"}</div>
            <div style={{ fontSize: 12, color: "var(--lcs-ink-muted)", marginTop: 4, maxWidth: 320, marginLeft: "auto", marginRight: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {visibleSessions.map((s) => {
              ...
              return (
                <div key={s.id} style={{ border: `1px solid var(--lcs-line)`, background: "var(--lcs-white)", padding: 20 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span style={{ fontSize: 12, fontWeight: 600, padding: "2px 10px", borderRadius: "var(--radius-lcs-control)", color: pill.text, background: pill.bg }}>
                        {pill.label}
                      </span>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 600, color: "var(--lcs-ink)" }}>Level {s.level} Roast</div>
                        <div style={{ fontSize: 12, color: "var(--lcs-ink-muted)", display: "flex", alignItems: "center", gap: 12, marginTop: 2 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      {s.status === "scheduled" && (
                        <>
                          <a href={`/app/roast/${s.id}/live`} style={{ display: "inline-flex", alignItems: "center", height: 32, padding: "0 12px", fontSize: 12, fontWeight: 500, color: "#fff", background: "#DC2626", borderRadius: "var(--radius-lcs-control)", textDecoration: "none" }}>
                            Go live →
                          </a>
                      ...
                      {s.status === "written_phase" && (
                        <a href={`/app/roast/${s.id}/answers`} style={{ display: "inline-flex", alignItems: "center", height: 32, padding: "0 12px", fontSize: 12, fontWeight: 500, color: "#fff", background: "#B45309", borderRadius: "var(--radius-lcs-control)", textDecoration: "none" }}>
                      ...
                      <a href={`/roast/${s.id}`} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 4, height: 32, padding: "0 12px", fontSize: 12, color: "var(--lcs-ink)", background: "var(--lcs-white)", border: `1px solid var(--lcs-line)`, borderRadius: "var(--radius-lcs-control)", textDecoration: "none" }}>
